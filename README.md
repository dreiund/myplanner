...
- **完成反馈沉淀**：任务完成时记录遇到的问题、可优化项和实际耗时，这些反馈会进入后续 AI 汇报上下文。
...
- **工作报告模式**：AI 汇报会聚焦「工作」分类任务，同时保留工作、学习、生活的分类统计信息。
- **自定义汇报人**：可在设置中配置汇报人名称，生成报告时自动写入 Markdown 模板。
...
如果你已经通过 `git clone` 安装过 MyPlanner，后续更新时不要重新 clone。进入原来的项目目录，拉取最新代码并重新安装依赖即可：
...
- `cd myplanner`：进入本地项目目录。
- `git pull`：从 GitHub 拉取最新代码。
- `npm ci`：按最新的 `package-lock.json` 重新安装依赖。
- `npm run dev`：启动 MyPlanner。

正常情况下，更新代码不会删除你的任务、复盘和设置数据。MyPlanner 的数据保存在系统的 Electron `userData` 目录中，不保存在项目源码目录里。
...
2. 填写遇到的问题和可以优化的地方。
3. 填写实际耗时，单位为分钟；如果不填写，任务仍可完成，只是不更新实际耗时。
4. 点击「确认完成」后，任务会被标记为已完成。
5. 已完成任务再次点击完成按钮，可以回退为未完成。
...
2. 填入「汇报人名称」，例如你的姓名或团队名称。
3. 点击「测试连接」确认 API Key 可用。测试会发起一次简短的 AI 请求。
4. 点击「保存」。
...
需要注意：AI 汇报中的任务明细会过滤为「工作」分类任务，适合直接生成工作日报、周报等正式汇报；统计区仍会展示工作、学习、生活的分类耗时或完成数，方便查看整体时间分布。
...
- 通过 `git pull` 和 `npm ci` 更新源码不会删除 `myplanner.db`，因为数据库不在项目源码目录中。
- 使用 AI 摘要时，当前报表范围内的任务、复盘、统计和反馈数据会发送给 DeepSeek API。
- 使用 AI 汇报时，工作类任务明细、复盘、统计数据、反馈内容和汇报人名称会发送给 DeepSeek API。
...
不要每次重新 `git clone`，否则电脑里会出现多个项目目录。MyPlanner 的数据库文件 `myplanner.db` 保存在 Electron 的 `userData` 目录里，不会因为源码目录执行 `git pull` 或 `npm ci` 被删除。
